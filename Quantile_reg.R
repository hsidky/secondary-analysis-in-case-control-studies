...
#  Note: the quantile regression package "quantreg" is required. The users can use the#
#  following codes to obtain "quantreg" package.        								  #
#  > install.packages("quantreg")                                                     #
#  > library(quantreg)                  		                                     	  #
#                                                                                     #
#  *function* 																		  #			  
#	-secondary_rq(formula, D, tau, data, pd_pop, iter=10, boot=0): obtain the point   #
#		estimate, bootstrap SE, Wald test statistics and p-values of the associations #
#		between the	continous secondary trait $Y$ and the genetic variants $X$        #
...
# 	-tau: the quantile level to be estimated. Multiple taus can be chosen simulaniouly# 	
...
# - create the sample dataset                                                        #                                                                       
	...
	colnames(dat)[1:2]=c("x1", "x2")                                                  #
# - run the proposed approach based on quantile regression                           #
  secondary_rq(y1~x1, D1, tau=0.5, dat, pd)                                     #
  secondary_rq(y1~x1+x2, D1, tau=c(0.25, 0.5), dat, pd)                             #
  secondary_rq(y1~x1, D1, tau=0.5, dat, pd, boot=10) 
  secondary_rq(y1~x1+x2, D1, tau=c(0.25, 0.5), dat, pd, boot=10)                   #
