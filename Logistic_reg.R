...
secondary_binary(y1~x1+x2, D1, dat, pd, boot=10)
...
secondary_binary <-function(formula, D, data, pd_pop, iter=10, boot=0) {
	...
	pred1= predict(pyD0, newdata=data.frame(temp.data[which(temp.data$D==1),])) # pseudo control
	pred0= predict(pyD1, newdata=data.frame(temp.data[which(temp.data$D==0),]))
	...
	pseudo=NULL
	for (iiter in 1:iter) {
		pseudo1=rbinom(n1, 1, py1) # d_i=1; D=1-d_i 
